...
# Load ChromaDB embeddings from local storage with optimizations
embeddings = OpenAIEmbeddings()

# Load ChromaDB retrievers with error handling and optimizations
try:
    # Use smaller k values for faster retrieval
    choice_retriever = Chroma(
        collection_name="choice_data_enums",
        embedding_function=embeddings,
        persist_directory="./data/chroma_db"
    )
    model_retriever = Chroma(
        collection_name="pydantic_model",
        embedding_function=embeddings,
        persist_directory="./data/chroma_db"
    )
    system_retriever = Chroma(
        collection_name="system_prompt",
        embedding_function=embeddings,
        persist_directory="./data/chroma_db"
    )
    collaboratory_retriever = Chroma(
        collection_name="collaboratory_activity_form",
        embedding_function=embeddings,
        persist_directory="./data/chroma_db"
    )
    user_retriever = Chroma(
        collection_name="user_prompt",
        embedding_function=embeddings,
        persist_directory="./data/chroma_db"
    )
    ...
    raise Exception("ChromaDB collections not found. Please generate embeddings first.")
...
# Optimized retrieval function with timing
def retrieve_text(query, retriever, k=3):  # Reduced from k=3 to k=2
...
def generate_activity(input_data: InputData):
    ...
    # Retrieve relevant texts with timing
    print("🔄 Starting retrieval...")
    choice_text = retrieve_text(input_text, choice_retriever, k=3)
    model_text = retrieve_text(input_text, model_retriever, k=3)
    system_text = retrieve_text(input_text, system_retriever, k=3)
    collaboratory_text = retrieve_text(input_text, collaboratory_retriever, k=3)
    user_text = retrieve_text(input_text, user_retriever, k=3)
